...
const App = () => {
  ...
  const showreelData = {
    ...
    desc: "The ultimate showcase of my creative journey. This showreel features a curated selection of my best work in Motion Graphics, Video Editing, and Visual Effects (VFX)."
  };
  ...
  const Navbar = () => (
    <nav className={`fixed top-0 w-full z-[60] transition-all duration-300 ${isScrolled ? 'bg-black/95 backdrop-blur-md py-4 border-b border-white/5 shadow-2xl' : 'bg-transparent py-8'}`}>
      ...
      <div className="max-w-7xl mx-auto px-6 flex justify-between items-center text-white text-left">
        <div className="flex items-center gap-4 cursor-pointer group" onClick={() => setCurrentPage('portfolio')}>
           ...
           <div className="hidden sm:block text-left text-white">
              <span className="text-lg font-black tracking-tighter block leading-none text-left mb-1">FALDANGG<span className="text-purple-500">.AEP</span></span>
              <span className="font-mono text-[8px] tracking-wider uppercase opacity-40 text-left block">Video Editor & Motion Designer</span>
           </div>
        </div>
  ...
  return (
    <div className="bg-[#050505] text-white min-h-screen selection:bg-purple-600 selection:text-white font-sans overflow-x-hidden relative text-left">
      ...
      {currentPage === 'portfolio' ? (
        <div className="portfolio-view">
          ...
          <section id="hero" className="relative min-h-screen flex items-center justify-center pt-20 px-6 text-center">
            ...
            <div className="relative z-10 text-center max-w-5xl mx-auto">
              ...
              <h1 className="text-[14vw] sm:text-[12vw] md:text-[8vw] font-black leading-[0.82] tracking-tighter uppercase mb-6 italic drop-shadow-2xl text-white">
        ...
        /* PRICES VIEW */
        <div className="prices-page min-h-screen bg-[#050505] text-white text-left text-left text-left">
          <section className="pt-40 pb-32 px-6 text-left text-left text-left">
            <div className="max-w-7xl mx-auto text-white text-left text-white text-left text-left">
              <div className="flex flex-col items-center text-center mb-20 text-left text-white text-left">
                 <div className="inline-flex items-center gap-3 px-4 py-2 bg-white/5 rounded-full border border-white/10 text-white/50 text-[10px] font-bold uppercase tracking-[0.4em] mb-6 shadow-xl text-left text-white text-left">
                    <Tag size={14} className="text-purple-500" /> Panduan Investasi
                 </div>
                 <h2 className="text-6xl md:text-8xl font-black uppercase italic tracking-tighter mb-6 text-center text-white text-center">{langAbout === 'ID' ? 'HARGA.' : 'PRICING.'}</h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-40 text-white text-left text-left text-left">
                  ...
                  <div key={i} className={`p-10 rounded-sm border transition-all duration-300 flex flex-col relative overflow-hidden text-left text-left text-left ${plan.popular ? 'border-purple-500 bg-[#111] shadow-2xl z-10 scale-105' : 'bg-white/5 border-white/5 opacity-80 hover:opacity-100'}`}>
                    ...
                    <div className="space-y-4 mb-12 flex-grow text-white text-left">
                      {plan.features.map((f, j) => (
                        <div key={j} className="flex items-center gap-3 text-xs font-bold uppercase tracking-wider text-white text-left text-white"><Check size={14} className="text-purple-500" strokeWidth={4} /> {f}</div>
                      ))}
                    </div>
              ...
              <div className="max-w-3xl mx-auto mt-32 text-white text-left text-left">
                <div className="text-center mb-16 text-white text-left text-center">
                   <p className="font-mono text-[10px] tracking-[0.4em] uppercase text-purple-500 mb-2 text-center text-white">Pertanyaan Umum</p>
                   <h2 className="text-4xl md:text-5xl font-black italic uppercase tracking-tighter text-center text-white text-center">FAQ.</h2>
                </div>
                <div className="space-y-4 text-left text-white text-left">
                  {faqs.map((faq, index) => (
                    <div key={index} className="rounded-sm border border-white/5 bg-[#0a0a0a] overflow-hidden transition-all shadow-lg text-left text-white text-left">
                      <button onClick={() => toggleFaq(index)} className="w-full px-8 py-6 flex items-center justify-between text-left group text-white text-left text-white">
                      ...
                      {activeFaq === index && <div className="px-8 pb-8 text-white/50 text-sm leading-relaxed font-medium text-left text-white text-left">{faq.a}</div>}
                    </div>
                  ))}
                </div>
              </div>
      ...
      {/* VIDEO MODAL */}
      {selectedVideo && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 md:p-10 text-white text-left text-left">
          <div className="absolute inset-0 bg-black/98 backdrop-blur-xl animate-fade-in text-left text-left text-left" onClick={() => setSelectedVideo(null)}></div>
          ...
          <div className={`relative w-full bg-[#0a0a0a] rounded-sm overflow-hidden shadow-[0_0_100px_rgba(0,0,0,0.8)] border border-white/10 flex flex-col max-h-[90vh] transition-all duration-300 ${selectedVideo.isVertical ? 'max-w-[400px]' : 'max-w-6xl'}`}>
            <div className="absolute top-0 left-0 w-full h-14 z-[130] flex items-center justify-between px-6 pointer-events-none text-left text-left">
               <span className="bg-purple-600 text-white text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-widest shadow-lg pointer-events-auto text-left text-left">{selectedVideo.category}</span>
               <button className="bg-black/50 p-2.5 rounded-full hover:bg-white hover:text-black transition-all shadow-lg active:scale-90 pointer-events-auto text-left text-left" onClick={() => setSelectedVideo(null)}><X size={18} /></button>
            </div>
            <div className={`w-full bg-black flex items-center justify-center relative overflow-hidden ${selectedVideo.isVertical ? 'aspect-[9/16]' : 'aspect-video'}`}>
              <iframe src={selectedVideo.videoUrl} className="w-full h-full border-0 text-left text-left text-left" title={selectedVideo.title} allowFullScreen></iframe>
            </div>
            <div className="p-8 md:p-10 bg-[#080808] border-t border-white/5 overflow-y-auto shrink-0 text-left text-white text-left text-left">
              <div className="flex flex-col text-white text-left text-left">
                <h2 className={`${selectedVideo.isVertical ? 'text-2xl' : 'text-3xl md:text-5xl'} font-black uppercase italic tracking-tighter text-white mb-4 text-left text-white text-left`}>{selectedVideo.title}</h2>
